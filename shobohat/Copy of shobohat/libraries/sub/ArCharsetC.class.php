...
// namespace I18N/Arabic/ArCharsetC;
...
class ArCharsetC
{
    ...
    {
        $handle = fopen(dirname(__FILE__).'/charset/charset.src', 'r');
        if ($handle) {
            $this->utfStr  = fgets($handle, 4096);
            $this->winStr  = fgets($handle, 4096);
            $this->isoStr  = fgets($handle, 4096);
            $this->htmlStr = fgets($handle, 4096);
            fclose($handle);
        }

        if (in_array('windows-1256', $sets)) {
            include dirname(__FILE__).'/charset/_windows1256.php';
        }
        
        if (in_array('iso-8859-6', $sets)) {
            include dirname(__FILE__).'/charset/_iso88596.php';
        }
        
        if (in_array('utf-8', $sets)) {
            include dirname(__FILE__).'/charset/_utf8.php';
        }
        
        if (in_array('bug', $sets)) {
            include dirname(__FILE__).'/charset/_bug.php';
        }
        
        if (in_array('html', $sets)) {
            include dirname(__FILE__).'/charset/_html.php';
        }
    }
    ...
    {
        return trim(substr($this->htmlStr, $index*4, 4));
    }
    ...
    {
        return trim(substr($this->utfStr, $index*2, 2));
    }
    ...
    protected function findUTF($char)
    {
        ...
        return strpos($this->utfStr, $char)/2;
    }
    ...
    {
        return substr($this->winStr, $index, 1);
    }
    ...
    protected function findWIN($char)
    {
        ...
        return strpos($this->winStr, $char);
    }
    ...
    {
        return substr($this->isoStr, $index, 1);
    }
    ...
    protected function findISO($char)
    {
        ...
        return strpos($this->isoStr, $char);
    }
